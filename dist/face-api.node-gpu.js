...
var Dimensions = class _Dimensions {
  ...
  reverse() {
    return new _Dimensions(1 / this.width, 1 / this.height);
  }
};
...
var Point = class _Point {
  ...
  add(pt) {
    return new _Point(this.x + pt.x, this.y + pt.y);
  }
  sub(pt) {
    return new _Point(this.x - pt.x, this.y - pt.y);
  }
  ...
  abs() {
    return new _Point(Math.abs(this.x), Math.abs(this.y));
  }
  magnitude() {
...
var Box = class _Box {
  ...
  toSquare() {
    ...
    return new _Box({ x, y, width, height });
  }
  rescale(s) {
  ...
  clipAtImageBorders(imgWidth, imgHeight) {
    ...
    return new _Box({ x: clippedX, y: clippedY, width: clippedWidth, height: clippedHeight }).floor();
  }
  shift(sx, sy) {
...
var ObjectDetection = class _ObjectDetection {
...
var FaceDetection = class _FaceDetection extends ObjectDetection {
  constructor(score, relativeBox, imageDims) {
    super(score, score, "", relativeBox, imageDims);
  }
  forSize(width, height) {
    const { score, relativeBox, imageDims } = super.forSize(width, height);
    return new _FaceDetection(score, relativeBox, imageDims);
  }
};
...
var LabeledFaceDescriptors = class _LabeledFaceDescriptors {
  ...
  static fromJSON(json) {
    ...
    return new _LabeledFaceDescriptors(json.label, descriptors);
  }
};
...
var DrawTextField = class _DrawTextField {
  constructor(text, anchor, options = {}) {
    this.text = typeof text === "string" ? [text] : text instanceof _DrawTextField ? text.text : text;
    this.anchor = anchor;
...
async function extractFaceTensors(imageTensor, detections) {
  ...
  return tf6.tidy(() => {
    ...
    const faceTensors = boxes.filter((box) => box.width > 0 && box.height > 0).map(({ x, y, width, height }) => tf6.slice3d(imageTensor.as3D(imgHeight, imgWidth, numChannels), [y, x, 0], [height, width, numChannels]));
    return faceTensors;
  });
}
...
var NeuralNetwork = class {
  ...
  async loadFromDisk(filePath) {
    ...
    const fetchWeightsFromDisk = (filePaths) => Promise.all(filePaths.map((fp) => readFile(fp).then((buf) => typeof buf === "string" ? Buffer.from(buf) : buf.buffer)));
...
function denseBlock3(x, denseBlockParams, isFirstLayer = false) {
  return tf10.tidy(() => {
    const out1 = tf10.relu(
      isFirstLayer ? tf10.add(
        tf10.conv2d(x, denseBlockParams.conv0.filters, [2, 2], "same"),
...
function denseBlock4(x, denseBlockParams, isFirstLayer = false, isScaleDown = true) {
  return tf10.tidy(() => {
    const out1 = tf10.relu(
      isFirstLayer ? tf10.add(
        tf10.conv2d(x, denseBlockParams.conv0.filters, isScaleDown ? [2, 2] : [1, 1], "same"),
        denseBlockParams.conv0.bias
...
function convLayer(x, params, padding = "same", withRelu = false) {
  return tf11.tidy(() => {
    const out = tf11.add(
      tf11.conv2d(x, params.filters, [1, 1], padding),
      params.bias
    );
...
function fullyConnectedLayer(x, params) {
  return tf16.tidy(() => tf16.add(
    tf16.matMul(x, params.weights),
    params.bias
  ));
}
...
function calculateFaceAngle(mesh) {
  ...
  const calcLengthBetweenTwoPoints = (a, b) => Math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2);
  const angle = {
    roll: void 0,
    pitch: void 0,
    yaw: void 0
  ...
  if (!mesh || !mesh.positions || mesh.positions.length !== 68)
    return angle;
...
var FaceLandmark68NetBase = class extends FaceProcessor {
  postProcess(output, inputSize, originalDimensions) {
    ...
    return tf21.tidy(() => {
      ...
      const landmarkTensors = output.mul(tf21.fill([batchSize, 136], inputSize, "float32")).sub(tf21.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(
        getPaddingX(batchIdx),
        getPaddingY(batchIdx)
      )))).div(tf21.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(
  ...
  async detectLandmarks(input) {
    const netInput = await toNetInput(input);
    const landmarkTensors = tf21.tidy(
      () => tf21.unstack(this.forwardInput(netInput))
    );
...
function extractorsFactory3(extractWeights, paramMappings) {
  function extractFilterValues(numFilterValues, numFilters, filterSize) {
    ...
    return tf25.tidy(
      () => tf25.transpose(
        tf25.tensor4d(weights, [numFilters, depth, filterSize, filterSize]),
        [2, 3, 1, 0]
      )
    );
  }
...
function extractParams5(weights) {
  ...
  const fc = tf25.tidy(
    () => tf25.transpose(tf25.tensor2d(extractWeights(256 * 128), [128, 256]), [1, 0])
  );
...
function outputLayer(boxPredictions, classPredictions, params) {
  return tf31.tidy(() => {
    ...
    let boxes = decodeBoxesLayer(
      tf31.reshape(tf31.tile(params.extra_dim, [batchSize, 1, 1]), [-1, 4]),
      tf31.reshape(boxPredictions, [-1, 4])
    );
...
function boxPredictionLayer(x, params) {
  return tf32.tidy(() => {
    ...
    const boxPredictionEncoding = tf32.reshape(
      convLayer(x, params.box_encoding_predictor),
      [batchSize, -1, 1, 4]
    );
    const classPrediction = tf32.reshape(
...
// src/tinyYolov2/TinyYolov2Base.ts
var _TinyYolov2Base = class _TinyYolov2Base extends NeuralNetwork {
  constructor(config) {
    super("TinyYolov2");
...
_TinyYolov2Base.DEFAULT_FILTER_SIZES = [3, 16, 32, 64, 128, 256, 512, 1024, 1024];
...
var FaceMatcher = class _FaceMatcher {
  ...
  static fromJSON(json) {
    ...
    return new _FaceMatcher(labeledDescriptors, json.distanceThreshold);
  }
};
